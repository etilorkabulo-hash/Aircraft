...
class GameView(context: Context) : SurfaceView(context), SurfaceHolder.Callback {
    ...
    private fun gameOver() {
        ...
        if (score > highScore) {
            pendingAchievements.add(Achievement.SCORE_10000) // Placeholder for high score achievement
        }
    }
    ...
    private fun drawGameOver(canvas: Canvas) {
        ...
        if (pendingAchievements.isNotEmpty()) {
            ...
            val achievementText = "Achievements Unlocked: ${pendingAchievements.size}"
            canvas.drawText(achievementText, screenWidth / 2, statsY + statsSpacing * 6, textPaint)
        }
    ...
    private fun drawTransition(canvas: Canvas) {
        // Transition effect between levels
    ...
    inner class GameThread(private val surfaceHolder: SurfaceHolder) : Thread() {
        ...
        override fun run() {
            ...
            val targetFrameTime = 1_000_000_000L / 60 // 60 FPS
