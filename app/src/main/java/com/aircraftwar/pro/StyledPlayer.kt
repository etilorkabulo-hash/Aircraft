...
class StyledPlayer(
    ...
    private fun drawShield(canvas: Canvas) {
        shieldPaint.shader = RadialGradient(
            0f, 0f, 70f,
            intArrayOf(Color.argb(80 + (shieldPulse * 100).toInt(), 0, 200, 255), Color.argb(40, 0, 150, 255), Color.argb(0, 0, 100, 255)),
            floatArrayOf(0f, 0.7f, 1f), Shader.TileMode.CLAMP
        )
        canvas.drawCircle(0f, 0f, 70f, shieldPaint)
        
        for (particle in shieldParticles) {
            val px = cos(particle.angle * PI / 180f) * particle.radius
            val py = sin(particle.angle * PI / 180f) * particle.radius
    ...
    private fun drawEngineParticles(canvas: Canvas) {
        ...
        for (particle in engineParticles) {
            paint.color = Color.argb(particle.alpha, sin(particle.y * 0.1f).toInt().coerceIn(0, 100) + 155, sin(particle.x * 0.1f).toInt().coerceIn(0, 100) + 155, 255)
